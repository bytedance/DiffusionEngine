...
class DetrDatasetMapper:
    ...
    def __init__(
        ...
        self.style = style
        self.style_desc = style_map[style]
    ...
    def __call__(self, dataset_dict):
        ...
        if 'captions' in dataset_dict:
            prompt = random.choice(dataset_dict['captions'])
            prompt = ' '.join(prompt.strip().split())
            prompt = prompt[:-1] if prompt.endswith('.') else prompt
            dataset_dict["prompt"] = f"{self.style_desc} of " + prompt.lower()
        else:
            assert "things" in dataset_dict, "Either captions or things should provided."
            sample = random.sample(dataset_dict['things'], k=min(len(dataset_dict['things']), 4))
            prompt = f"a {self.style}, with " + ', '.join(sample) + f' in the {self.style_desc}.'
            dataset_dict["prompt"] = prompt
        dataset_dict["style"] = self.style
        return dataset_dict
