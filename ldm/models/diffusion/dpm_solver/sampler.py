...
class DPMSolverSampler(object):
    ...
    def sample(self,
        ...
        model_fn = model_wrapper(
            lambda x, t, c: self.model.apply_model(x, t, c, **kwargs),
            self.noise_schedule,
            model_type="noise",
        ...
        timesteps = dpm_solver.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=S, device=device)
        t_thr = int(timesteps[int(.8 * S)] * dpm_solver.noise_schedule.total_N - 1)
        kwargs['ts_thr'] = t_thr
    ...
    @torch.no_grad()
    def inverse(self, x0, steps, scale, c, uc):
        kwargs = {'edit': False}
        model_fn = model_wrapper(
            lambda x, t, c: self.model.apply_model(x, t, c, **kwargs),
            self.noise_schedule,
        ...
        x = dpm_solver.inverse(x0, steps=steps)
        return x
